...
$(function() {
    ...
    $(".Qd").click(function() {
        ...
        $.ajax({
            type: "POST",
            url: "/app4friendv2/member_add.jspx",
            data: {
                'member.mobile': user_nam,
                'member.password': user_pas,
                'member.token': 'abc123'
            },
            async: true,
            success: function(data) {
                data = JSON.parse(data);
                if (data.member_add.result == "0") {
                    alert('该账户以注册,请登录');
                    window.location = "index.html"
                    window.localStorage.setItem("nam", $(".b1").val());
                    window.localStorage.setItem("pas", $(".b4").val());
                } else {
                    window.location = "zucexinxi.html";
                }
            },
